...
# Default file_name / sha256_hash for the native case (host == build).
# Selects the right native tarball based on the detected build arch/OS.
# Use := so the value is captured at file-include time (while $(package)
# still refers to native_rust). The cross-compile branch below overrides
# $(package)_exact_file_name / $(package)_exact_sha256_hash using the
# rust-std sysroot mapping.
$(package)_build_os_key:=$(if $(filter aarch64,$(build_arch)),$(if $(filter linux,$(build_os)),aarch64_linux,$(build_os)),$(build_os))
$(package)_file_name:=$($(package)_file_name_$($(package)_build_os_key))
$(package)_sha256_hash:=$($(package)_sha256_hash_$($(package)_build_os_key))

# Mapping from GCC canonical hosts to Rust targets
# If a mapping is not present, we assume they are identical, unless $host_os is
# "darwin", in which case we map to the appropriate Apple triple by arch.
...
# Resolve the Rust target triple unconditionally so depends/Makefile's
# @rust_target@ substitution for config.site always has a value, and so
# the cross branch below can build $(package)_exact_file_name without
# re-referencing the (recursive) rust_target macro later.
