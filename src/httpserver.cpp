...
static bool HTTPBindAddresses(struct evhttp* http)
{
    ...
    for (const std::string& strAlgoPort : gArgs.GetArgs("-rpcalgoport")) {
        ...
        // Bind on loopback (same security model as default RPC)
        evhttp_bound_socket* bind_handle = evhttp_bind_socket_with_handle(http, "127.0.0.1", port);
        if (bind_handle) {
            boundSockets.push_back(bind_handle);
            g_rpc_algo_ports[port] = algo;
            LogPrintf("Binding RPC algo %s on port %i\n", algoName, port);
        } else {
            LogPrintf("Binding RPC algo %s on port %i failed.\n", algoName, port);
        }
        // Also bind IPv6 loopback
        evhttp_bound_socket* bind_handle6 = evhttp_bind_socket_with_handle(http, "::1", port);
        if (bind_handle6) {
            boundSockets.push_back(bind_handle6);
        }
    }
