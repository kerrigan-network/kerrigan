                                 ...
                                 const Consensus::Params& params, bool fJustCheck)
{
    ...
    // This allows ReplayBlocks rollforward to succeed without reindex (#1007).
    {
        LOCK(cs);
    ...
    // Discard any stale pending batch from a previous failed ConnectBlock (#532).
    {
        LOCK(cs);
        if (m_pendingBatch) {
    ...
    // Load the frontier from the cached last-written height (#545).
    ...
    // Cross-check frontier root against stored anchor (#606)
    if (m_lastFrontierHeight >= 0) {
        uint256 storedRoot;
                              ...
                              const Consensus::Params& params)
{
    ...
    if (nullifiers.empty() && commitments.empty()) {
        ...
        // Recompute frontier height cache if we undid the tip (#545)
