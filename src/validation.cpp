...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // Growth escrow: reject any tx spending from the consensus-locked escrow address
    {
            ...
            for (const auto& txin : tx.vin) {
                ...
                if (!coin.IsSpent() && coin.out.scriptPubKey == escrowScript) {
                    return state.Invalid(TxValidationResult::TX_CONSENSUS,
                        "bad-growth-escrow-spend", "spending from consensus-locked growth escrow is not permitted");
                }
            }
        }
    }
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool fVerifyOnly)
{
    ...
    // the second block walked because fVerifyOnly prevents state updates. (#805)
    if (!fVerifyOnly &&
        ...
        // handles this idempotently (#1012, mirrors #1007 connect-path fix).
                               ...
                               CCoinsViewCache& view, bool fJustCheck, bool fVerifyOnly)
{
    ...
    if (DeploymentActiveAt(*pindex, m_params.GetConsensus(), Consensus::DEPLOYMENT_HMP)) {
        ...
        // Determine the active HMP stage based on height alone.
        const auto& hmpConsensus = m_params.GetConsensus();
        int nHMPStage;
        if (pindex->nHeight < hmpConsensus.nHMPStage2Height) {
            nHMPStage = 1;
        } else if (pindex->nHeight < hmpConsensus.nHMPStage3Height) {
            nHMPStage = 2;
        } else if (pindex->nHeight < hmpConsensus.nHMPStage4Height) {
            nHMPStage = 3;
        } else {
            nHMPStage = 4;
        }
        ...
        if (nHMPStage == 1) {
            ...
            if (cbTxOpt && cbTxOpt->nVersion >= CCbTx::Version::HMP_SEAL) {
                ...
                if (nHMPStage >= 3) {
                    ...
                    // Consensus-level commitment validation: strip uncommitted signers from seal.
                    // Maturity is checked against the sealed ancestor height (not the embedding block),
                    // because signers must be committed before the block they're sealing.
                    if (g_hmp_commitments && g_hmp_commitments->IsEnabled() && !sealSignerPubKeys.empty()) {
                        int sealedHeight = pindex->nHeight - hmpConsensus.nHMPSealTrailingDepth;
                        std::vector<CBLSPublicKey> committedSigners;
                        std::vector<uint8_t> committedAlgos;
                        committedSigners.reserve(sealSignerPubKeys.size());
                        committedAlgos.reserve(sealSignerAlgos.size());
                        for (size_t i = 0; i < sealSignerPubKeys.size(); i++) {
                            if (g_hmp_commitments->IsCommitted(sealSignerPubKeys[i], sealedHeight)) {
                                committedSigners.push_back(sealSignerPubKeys[i]);
                                if (i < sealSignerAlgos.size()) {
                                    committedAlgos.push_back(sealSignerAlgos[i]);
                                }
                            } else {
                                LogPrint(BCLog::NET, "HMP: ignoring uncommitted seal signer %s at sealed height %d\n",
                                         sealSignerPubKeys[i].ToString().substr(0, 16), sealedHeight);
                            }
                        }
                        sealSignerPubKeys = std::move(committedSigners);
                        sealSignerAlgos = std::move(committedAlgos);
                    }

                    // Validate signerAlgos against privilege tracker.
                    // The BLS signature cryptographically binds algoId; if the signer has no
                    // privilege on the algo they signed with, reject them entirely rather than
                    // reassigning to a different algo (which would be non-deterministic).
                    if (g_hmp_privilege && !sealSignerPubKeys.empty()) {
                        for (size_t i = 0; i < sealSignerPubKeys.size() && i < sealSignerAlgos.size(); i++) {
                            HMPPrivilegeTier tier = g_hmp_privilege->GetTier(sealSignerPubKeys[i], sealSignerAlgos[i]);
                            if (tier == HMPPrivilegeTier::UNKNOWN) {
                                // No privilege on the signed algo, remove this signer.
                                // Do NOT reassign to a different algo: that would depend on local
                                // privilege state and cause consensus divergence across nodes.
                                LogPrint(BCLog::NET, "HMP: removing signer %s with no privilege on algo %d at height %d\n",
                                         sealSignerPubKeys[i].ToString().substr(0, 16),
                                         sealSignerAlgos[i], pindex->nHeight);
                                sealSignerPubKeys.erase(sealSignerPubKeys.begin() + i);
                                sealSignerAlgos.erase(sealSignerAlgos.begin() + i);
                                i--;
                            }
                        }
                    }
                    ...
                    // Remove from candidates before mutating sort key to avoid std::set UB (#1071).
                    // Skip during fJustCheck (TestBlockValidity) — pindex is a stack-local
                    // indexDummy that must not be inserted into the global set (#1078).
                    if (!fJustCheck) {
                        setBlockIndexCandidates.erase(pindex);
                    }
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            // Growth escrow: consensus-locked, no spending allowed without governance
            {
                ...
                if (!escrowScriptVec.empty()) {
                    ...
                    for (const auto& txin : tx.vin) {
                        const Coin& coin = view.AccessCoin(txin.prevout);
                        if (!coin.IsSpent() && coin.out.scriptPubKey == escrowScript) {
                            LogPrintf("ERROR: %s: tx %s attempts to spend from consensus-locked growth escrow\n",
...
bool CChainState::DisconnectTip(BlockValidationState& state, DisconnectedBlockTransactions* disconnectpool)
{
    ...
    // HMP: disconnect commitment registry, privilege tracker, and seal manager
    if (g_hmp_commitments) {
...
bool CChainState::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    // scenarios since different nodes can disagree on fork-choice (#817).
...
bool CChainState::InvalidateBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    ...
    // #708: Use nChainSealWork (HMP-weighted) for candidate lookup
...
static bool CheckKawPowSolution(const CBlockHeader& block, BlockValidationState& state)
{
    ...
    // Epoch context grows ~8 MB per epoch; epoch 2048 ≈ 60 years of blocks.
    ...
    // block.GetHash() returns X11 which no KawPoW miner uses. (#801)
...
static bool CheckBlockHeader(const CBlockHeader& block, const uint256& hash, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    if (fCheckPOW) {
        ...
        if (algo == ALGO_EQUIHASH_200 || algo == ALGO_EQUIHASH_192) {
            ...
            // #560: Context-free path only checks epoch bounds. Full
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    for (const auto& tx : block.vtx) {
        // Early valueBalance range check (#557)
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (block.GetAlgo() == ALGO_KAWPOW) {
        ...
        // #560: Full KawPoW solution verify with confirmed nHeight
    ...
    // Algo rate limit removed (#1035): DAA + HMP seal weight handle rebalancing.
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // there's no median time past without a parent. (#791)
...
void CChainState::RebuildHMPState()
{
    ...
    for (int height = startHeight; height <= tip->nHeight; height++) {
        ...
        // Determine HMP stage for this height
        int nHMPStage;
        if (height < consensus.nHMPStage2Height) {
            nHMPStage = 1;
        } else if (height < consensus.nHMPStage3Height) {
            nHMPStage = 2;
        } else if (height < consensus.nHMPStage4Height) {
            nHMPStage = 3;
        } else {
            nHMPStage = 4;
        }
        ...
        for (const auto& tx : block.vtx) {
            if (tx->nVersion == 3 && tx->nType == TRANSACTION_COINBASE) {
                ...
                if (cbTxOpt && cbTxOpt->nVersion >= CCbTx::Version::HMP_SEAL) {
                    ...
                    // Filter signers (same logic as RollforwardBlock)
                    ...
                    if (cbTxOpt->sealForAncestor.IsValid() && !cbTxOpt->sealBlockHash.IsNull()) {
                        ...
                        // Strip uncommitted signers
                        if (g_hmp_commitments && g_hmp_commitments->IsEnabled() && !filteredSigners.empty()) {
                            int sealedHeight = height - consensus.nHMPSealTrailingDepth;
                            std::vector<CBLSPublicKey> committed;
                            std::vector<uint8_t> committedAlgos;
                            committed.reserve(filteredSigners.size());
                            committedAlgos.reserve(filteredAlgos.size());
                            for (size_t i = 0; i < filteredSigners.size(); i++) {
                                if (g_hmp_commitments->IsCommitted(filteredSigners[i], sealedHeight)) {
                                    committed.push_back(filteredSigners[i]);
                                    if (i < filteredAlgos.size())
                                        committedAlgos.push_back(filteredAlgos[i]);
                                }
                            }
                            filteredSigners = std::move(committed);
                            filteredAlgos = std::move(committedAlgos);
                        }

                        // Strip unprivileged signers
                        if (g_hmp_privilege && !filteredSigners.empty()) {
                            for (size_t i = 0; i < filteredSigners.size() && i < filteredAlgos.size(); i++) {
                                HMPPrivilegeTier tier = g_hmp_privilege->GetTier(filteredSigners[i], filteredAlgos[i]);
                                if (tier == HMPPrivilegeTier::UNKNOWN) {
                                    filteredSigners.erase(filteredSigners.begin() + i);
                                    filteredAlgos.erase(filteredAlgos.begin() + i);
                                    i--;
                                }
                            }
                        }
        ...
        for (const auto& tx : block.vtx) {
            if (tx->nVersion == 3 && tx->nType == TRANSACTION_COINBASE) {
                ...
                if (cbTxOpt && cbTxOpt->nVersion >= CCbTx::Version::HMP_SEAL) {
                    ...
                    // Filter signers the same way ConnectBlock does.
                    std::vector<CBLSPublicKey> filteredSigners;
                    std::vector<uint8_t> filteredAlgos;
                    if (cbTxOpt->sealForAncestor.IsValid() && !cbTxOpt->sealBlockHash.IsNull()) {
                    ...
                    // Strip uncommitted signers; maturity checked against sealed ancestor
                    if (g_hmp_commitments && g_hmp_commitments->IsEnabled() && !filteredSigners.empty()) {
                        int sealedHeight = pindex->nHeight - hmpConsensus.nHMPSealTrailingDepth;
                        std::vector<CBLSPublicKey> committed;
                        std::vector<uint8_t> committedAlgos;
...
bool CChainState::ReplayBlocks()
{
    ...
    if (!hashHeads[1].IsNull()) { // The old tip is allowed to be 0, indicating it's the first flush.
        ...
        // skips blocks already committed to SaplingDB (#611, #1007).
    ...
    while (pindexOld != pindexFork) {
        if (pindexOld->nHeight > 0) { // Never disconnect the genesis block.
            ...
            // Disconnect HMP state to match DisconnectTip behavior (#450)
