...
{
    argsman.AddArg("-autorebuildsaplingwitnesses", "Automatically detect and rebuild stale Sapling per-note witnesses once the node finishes its initial block download (default: 1). Disable with -noautorebuildsaplingwitnesses to skip the check and run z_rebuildsaplingwitnesses manually when needed.", ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
