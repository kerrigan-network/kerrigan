    ...
    return RPCHelpMan{"z_getnewaddress",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            if (km.IsEmpty()) {
                ...
                // Capture master key for encrypted wallets (#480)
...
RPCHelpMan z_sendmany()
{
    return RPCHelpMan{"z_sendmany",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            // #453: Hold cs_wallet only for input gathering (not during Groth16 proof generation).
            ...
            std::set<std::string> seenAddresses; // #868: detect duplicate recipients
            for (unsigned int i = 0; i < amounts.size(); i++) {
                ...
                // #463: check BEFORE adding to prevent signed overflow UB
            ...
            // Note cmus reserved for this tx; unreserved on completion (#499).
            ...
            if (fromIsShielded) {
                ...
                // Cleanse spending key on all exit paths (#1081)
                ...
                if (request.params[3].isNull()) {
                    ...
                    for (int iter = 0; iter < 10 && estimatedFee != prevFee; ++iter) {
                        ...
                        // Builder pads outputs to min 2 for privacy (#502)
            ...
            // Privacy fix (#80): Do NOT apply the sender's OVK to external
            ...
            //    Check both payload size and total serialized tx size (#621).
            ...
            // #455: CommitTransaction throws std::runtime_error on db failure;
...
RPCHelpMan z_importkey()
{
    return RPCHelpMan{"z_importkey",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            // #458: cleanse the hex string immediately after decoding
...

            {
                ...
                // Capture master key for encrypted wallets (#480): spending keys must be
...
RPCHelpMan z_importviewingkey()
{
    return RPCHelpMan{"z_importviewingkey",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            // Cleanse decoded key material (#785)
            memory_cleanse(keyHex.data(), keyHex.size());
            memory_cleanse(keyBytes.data(), keyBytes.size());
