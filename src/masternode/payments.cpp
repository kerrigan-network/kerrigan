                                                            ...
                                                            const CAmount feeReward)
{
    ...
    // Devnet genesis (h=1) is auto-generated by FindDevNetGenesisBlock as a
    // single OP_RETURN coinbase with no treasury split, so it cannot satisfy
    // the 5-way payment check. Skip the check for that one block only.
    if (Params().NetworkIDString() == CBaseChainParams::DEVNET && nBlockHeight == 1) {
        return true;
    }
