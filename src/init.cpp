...
static bool DownloadSaplingParam(const std::string& filename, const fs::path& dest, const std::string& expected_sha256)
{
    ...
    for (const auto& url : urls) {
        ...
        int ret = -1;
#ifdef WIN32
        // #825: Use CreateProcessA instead of system() to avoid shell-injection via dest path.
        // Build PowerShell arguments as a proper argv-style command line.
        {
            ...
            // #894: Also escape double-quotes to prevent breaking out of the outer PowerShell quoting
...
#else
        // #697: Use fork/execvp instead of system() to avoid shell injection via dest path
...
void PrepareShutdown(NodeContext& node)
{
    ...
    // Stop seal manager AFTER m_load_block joins and script check workers stop (#947).
...
* the restart path where it may have been called once already (#948).
*/
void Shutdown(NodeContext& node)
{
    // Shutdown part 1: always flush + stop threads first.
    // PrepareShutdown is idempotent (uses a mutex), so it's safe to call
    // even on the restart path where it was already called once (#948).
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    // ********************************************************* Step 7a2: Initialize HMP identity + Groth16 params
    {
        ...
        if (chainparams.GetConsensus().nHMPCommitmentOffset > 0) {
            ...
            // SPORK_25_HMP_ENABLED gates commitment broadcasting (#615).
    ...
    // Rebuild HMP in-memory tracker state from the active chain (#816).
