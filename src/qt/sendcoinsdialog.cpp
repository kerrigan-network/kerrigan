...
void SendCoinsDialog::sendShielded(const QList<SendCoinsRecipient>& recipients, bool relockOnComplete)
{
    ...
    connect(worker, &ShieldedSendWorker::finished, this, [this, relockOnComplete, progress, thread, worker](const QString& txid) {
        progress->close();
        progress->deleteLater();
        if (relockOnComplete && model) {
    ...
    connect(worker, &ShieldedSendWorker::error, this, [this, relockOnComplete, progress, thread, worker](const QString& message) {
        progress->close();
        progress->deleteLater();
        if (relockOnComplete && model) {
            model->setWalletLocked(true);
...
void SendCoinsDialog::sendButtonClicked([[maybe_unused]] bool checked)
{
    ...
    if (hasShieldedRecipient(allRecipients)) {
        ...
        if (encStatus == model->Locked) {
            ...
            try {
                ...
                model->setWalletLocked(true); // Re-lock on exception (#665)
                fNewRecipientAllowed = true;
                throw;
            }
            return;
    ...
    if (retval == QMessageBox::Save) {
        ...
        // Copy PSBT to clipboard and offer to save (#594: fall through to cleanup)
        if (!send_failure && !complete) {
        // Serialize the PSBT
...
{
    // Check that we can accept new recipients before pasting (#593)
    if (!fNewRecipientAllowed)
        return false;
    pasteEntry(rv);
    return true;
