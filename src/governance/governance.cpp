...
void CGovernanceManager::AddPostponedObjectInternal(const CGovernanceObject& govobj)
{
    ...
    // #462: cap mapPostponedObjects to prevent memory exhaustion via pending-collateral flood
    ...
    for (auto it = mapObjects.begin(); it != mapObjects.end();) {
        ...
        if ((pObj->IsSetCachedDelete() || pObj->IsSetExpired()) &&
            ...
            if (pObj->GetObjectType() == GovernanceObject::PROPOSAL) {
                ...
                // tombstone-bypassed re-submission. (#362)
            ...
            // #528: Cap erased objects map to prevent unbounded growth
...
bool CGovernanceManager::ConfirmInventoryRequest(const CInv& inv)
{
    ...
    // Evict begin() entry (deterministic by hash order, O(1)) instead of O(n) scan (#901, #938)
