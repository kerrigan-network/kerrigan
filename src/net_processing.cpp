...
void PeerManagerImpl::FinalizeNode(const CNode& node) {
    ...
    // Clean up per-NodeId HMP rate-limit maps on disconnect (#R9)
    {
    ...
    const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::GETMNLISTDIFF) {
        ...
        {
            static std::atomic<int64_t> g_last_mnlistdiff_time{0};
            int64_t nNow = GetTime();
            if (nNow == g_last_mnlistdiff_time.load()) {
                LogPrint(BCLog::NET, "GETMNLISTDIFF global rate limit hit, peer=%d\n", pfrom.GetId());
                return;
            }
            g_last_mnlistdiff_time.store(nNow);
        }
    ...
    if (msg_type == NetMsgType::GETQUORUMROTATIONINFO) {
        ...
        {
            static std::atomic<int64_t> g_last_qrinfo_time{0};
            int64_t nNow = GetTime();
            if (nNow == g_last_qrinfo_time.load()) {
                LogPrint(BCLog::NET, "GETQUORUMROTATIONINFO global rate limit hit, peer=%d\n", pfrom.GetId());
                return;
            }
            g_last_qrinfo_time.store(nNow);
        }
    ...
    if (found)
    {
        ...
        if (msg_type == NetMsgType::SEALSHARE) {
            ...
            if (m_chainman.ActiveChainstate().IsInitialBlockDownload()) return;
            // Skip processing when SPORK_25 kill switch is engaged.
        ...
        if (msg_type == NetMsgType::PUBKEYCOMMIT) {
            // Skip HMP messages during IBD.
            if (m_chainman.ActiveChainstate().IsInitialBlockDownload()) return;
            // Skip when SPORK_25 kill switch is engaged or spork manager unavailable (#1069)
