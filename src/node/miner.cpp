...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    // #539: nBits param is unused by Kerrigan's height-only subsidy, kept for API compat.
    ...
    if (!fDIP0003Active_context) {
        ...
        // #677: minerIdentity is consensus-required at Stage 3+ and must be
        ...
        // SPORK_25_HMP_ENABLED gates seal/commitment production only (#615).
        if (fHMPActive_context && g_sporkman && g_sporkman->IsSporkActive(SPORK_25_HMP_ENABLED)) {
            if (g_seal_manager) {
                ...
                if (seal) {
                    ...
                    // without in-memory seal cache after restart (#471).
