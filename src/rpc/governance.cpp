        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // #529: Cap input size to prevent unbounded allocation from ParseHex
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (strDataHex.size() > 32768) {
        throw JSONRPCError(RPC_INVALID_PARAMETER,
            strprintf("hex_data too large: %u bytes (max 32768)", strDataHex.size()));
    }
