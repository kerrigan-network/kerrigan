        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // #452: clamp count to [1, 200] to prevent negative/huge values
    ...
    for (int h = nStartHeight; h <= nChainTipHeight; h++) {
        if (h < 1) continue; // #521: guard against GetAncestor(-1) at genesis
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // #451/#456: clamp count to [1, 200] to prevent INT64_MIN UB and unbounded loop
    ...
    // #456: obtain mempool reference once before the loop (not every block iteration)
